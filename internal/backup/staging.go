...
func Stage(ctx context.Context, rt engine.Runtime, target engine.Container, cfg config.BackupConfig) (*StagedBackup, error) {
	if cfg.Source != "" {
...
func repoFromS3URI(uri string) (config.RepoConfig, error) {
	trimmed := strings.TrimPrefix(uri, "s3://")
	bucket, key, ok := strings.Cut(trimmed, "/")
	if !ok || bucket == "" || key == "" {
		return config.RepoConfig{}, fmt.Errorf("invalid s3 source %q (use s3://bucket/key-or-prefix)", uri)
	}
	return config.RepoConfig{Type: "s3", Bucket: bucket, Prefix: key}, nil
}
...
func downloadS3(ctx context.Context, repo config.RepoConfig, w io.Writer) (string, error) {
	region := repo.Region
	if region == "" {
		region = "us-east-1"
	}
