...
func buildReporter(format string, cfg *config.Config, out io.Writer) engine.Reporter {
...
func webhookAlertURL(alert config.AlertSpec) string {
	if alert.URL != "" {
		return alert.URL
	}
...
func writeConfiguredReports(ctx context.Context, cfg config.ReportConfig, results []engine.DrillResult, currentRun *state.LastRun) error {
