...
func TestRestoreRDBStagesBackupAndCapturesLastSave(t *testing.T) {
	...
	result, err := New().Restore(context.Background(), rt, config.BackupConfig{
		Tool:   "rdb",
		Source: path,
	}, fakeContainer{})
	if err != nil {
...
func TestRestoreRejectsUnsupportedTool(t *testing.T) {
	_, err := New().Restore(context.Background(), &redisRestoreRuntime{}, config.BackupConfig{Tool: "snapshot"}, fakeContainer{})
	if err == nil {
...
func TestValidateRedisQueryRejectsEmptyCommand(t *testing.T) {
	result, err := New().Validate(context.Background(), &redisValidateRuntime{}, fakeContainer{}, []config.Check{
