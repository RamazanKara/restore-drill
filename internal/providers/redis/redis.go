...
func (p *Provider) restoreRDB(ctx context.Context, rt engine.Runtime, cfg config.BackupConfig, target engine.Container) (*engine.RestoreResult, error) {
...
func (p *Provider) restoreAOF(ctx context.Context, rt engine.Runtime, cfg config.BackupConfig, target engine.Container) (*engine.RestoreResult, error) {
	staged, err := backup.Stage(ctx, rt, target, cfg)
	if err != nil {
		return nil, err
	}
