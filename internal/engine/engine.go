...
func (e *Engine) Run(ctx context.Context, drills []config.DrillConfig) ([]DrillResult, error) {
...
func (e *Engine) RunParallel(ctx context.Context, drills []config.DrillConfig) ([]DrillResult, error) {
	results := make([]DrillResult, len(drills))
	var wg sync.WaitGroup

	for i, drill := range drills {
		wg.Add(1)
		go func(idx int, d config.DrillConfig) {
			defer wg.Done()
...
func (e *Engine) executeDrill(ctx context.Context, drill config.DrillConfig) (result DrillResult) {
	result = DrillResult{
		Name:      drill.Name,
	...
	spec := ContainerSpec{
		...
		Env:   env,
		Ports: config.GetDefaultPorts(drill.Provider),
	}
