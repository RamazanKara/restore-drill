...
func TestWebhookAlertURL(t *testing.T) {
	tests := []struct {
		name  string
		alert engine.AlertSpec
		want  string
	}{
		{name: "prefers url", alert: engine.AlertSpec{URL: "https://hook", Endpoint: "https://old"}, want: "https://hook"},
		{name: "falls back to endpoint", alert: engine.AlertSpec{Endpoint: "https://old"}, want: "https://old"},
		{name: "empty when neither set", alert: engine.AlertSpec{}, want: ""},
	}
